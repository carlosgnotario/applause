var AppModule = (() => {
  ...
  var Swiper = class {
    ...
    animateIntro() {
      ...
      gsapWithCSS.set(this.slides, {
        opacity: 0
      });
      filteredSlides.forEach((slide, index) => {
        const edge = -((this.totalWidth - this.swiperWidth) / 2 - this.slideWidth / 2);
        if (slide.left + slide.width + this.centeringOffset + slide.loop * this.totalWidth < edge) {
          slide.loop += 1;
        }
        if (slide.left + slide.width + this.centeringOffset + (slide.loop - 1) * this.totalWidth >= edge) {
          slide.loop -= 1;
        }
    ...
    changeSlide(slide) {
      ...
      if (this.options.controls && !this.options.loop) {
        if (slide >= this.slides.length - 1) {
          ...
          gsapWithCSS.to(this.controls.querySelector(".prev"), {
            opacity: 0.2,
            duration: 0.5
          });
        } else {
        ...
        gsapWithCSS.to(this.slides[prevSlide].querySelector(".swiper-slide-content"), {
          opacity: 0,
          y: 40
        });
      }
    ...
    interactions() {
      this.cards.forEach((card) => {
        card.addEventListener("mouseenter", () => {
          gsapWithCSS.to(card.titleElement, {
            y: -card.content.offsetHeight - 10,
            duration: 1.2
          });
          gsapWithCSS.to(card.content, {
            opacity: 1,
            duration: 1.2,
            y: 0
          });
        ...
        card.addEventListener("mouseleave", () => {
          gsapWithCSS.killTweensOf(card.titleElement);
          gsapWithCSS.killTweensOf(card.content);
          gsapWithCSS.to(card.titleElement, {
            y: 0
          });
          gsapWithCSS.to(card.content, {
  ...
  var FAQ = class {
    ...
    openItem(item) {
      ...
      gsapWithCSS.to(this.item[item].answer, {
        height: "auto",
        duration: 0.5
      });
  ...
  // js/classes/ScrollSlider.js
  var ScrollSlider = class {
    constructor(element) {
      this.element = element;
      this.init();
      gsapWithCSS.registerPlugin(ScrollTrigger2);
    ...
    update() {
      const sliderBarW = gsapWithCSS.quickTo(this.slideBar, "widthPercent");
    ...
    changeSlide(slide) {
      ...
      if (this.currentSlide !== null) {
        ...
        const oldTL = gsapWithCSS.timeline();
        oldTL.to(oldSlide, {
          autoAlpha: 0
        });
      ...
      const newTL = gsapWithCSS.timeline();
      newTL.to(newSlide, {
        autoAlpha: 1
      });
